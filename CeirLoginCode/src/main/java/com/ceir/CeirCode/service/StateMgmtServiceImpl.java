...
public class StateMgmtServiceImpl {
	...
	public List<StatesInterpretationDb> findByFeatureId(Integer featureId){ 
		try
		{	
			List<StatesInterpretationDb> stateMgmtDbsResult = new ArrayList<>();
			logger.info("Going to get states by featureId "+featureId);
			List<StatesInterpretationDb> list = statesInterpretaionRepository.findByFeatureId(featureId);
			logger.debug(list);
			logger.info("states in response "+list);
			
			for(StatesInterpretationDb data : list) {
				data.getState();
				data.getInterp();
				stateMgmtDbsResult.add(data);
