...
public class RegisterController {
	 ...
	 * 
	 * 
	 * @GetMapping({ "/openUserRegisterPage" }) public ModelAndView
	 * openUserRegisterPage(HttpSession session) { ModelAndView mv = new
	 * ModelAndView(); if(session.getAttribute("username")!=null) {
	 * mv.setViewName("redirect:/Dashboard");
	 * System.out.println("000000"+session.getAttribute("username")); } else {
	 * System.out.println("inside method"); mv.setViewName("StartingPage"); } return
	 * mv; }
	 * 
	 * 
	 * 
	 ...
	 * {"/importerConsignment/{id}"},method={org.springframework.web.bind.annotation
	 * .RequestMethod.GET}) public ModelAndView demo(@PathVariable("id") int
	 * id,HttpSession session) { System.out.println("inside dashboard method");
	 * ModelAndView mv = new ModelAndView();
	 ...
	 * *****************************************************
	 * 
	 * @RequestMapping(value="/addConsignment",method=RequestMethod.POST) public
	 * ModelAndView addConsignment(@ModelAttribute ConsignmentPojo
	 * consignmentFormData,@RequestParam("file") MultipartFile
	 * file,HttpServletRequest request) {
	 * 
	 * Enumeration<String> headerNames = request.getHeaderNames();
	 * while(headerNames.hasMoreElements()) { String paramName =
	 * (String)headerNames.nextElement(); System.out.println("Headername==" +
	 * paramName ); String paramValue = request.getHeader(paramName);
	 * System.out.println("Header Value==" +paramValue ); }
	 * 
	 * ModelAndView mv = new ModelAndView();
	 * System.out.println("inside addConsignment method"); try {
	 * 
	 * System.out.println(" multipart filename="+file.getOriginalFilename()
	 * +" file size="+file.getSize()+" bytes details"+file.getBytes()+" name="+file.
	 * getName()+" Content type="+file.getContentType()); mv.setViewName("demo");
	 * 
	 ...
	 * 
	 * byte[] bytes = file.getBytes(); String rootPath =
	 * "/home/ubuntu/apache-tomcat-9.0.4/webapps/Design/"; File dir = new
	 ...
	 * *****************************************************
	 * 
	 * @RequestMapping(value="/updateConsignmentDetail/{txnid}",method=RequestMethod
	 * .POST) public ModelAndView updateConsignment(@ModelAttribute ConsignmentPojo
	 * consignmentFormData,@RequestParam(name="file",required =false) MultipartFile
	 * file,HttpServletRequest request,@PathVariable("txnid") String txnid) {
	 * 
	 * 
	 * consignmentFormData.setImporterId(1);
	 * consignmentFormData.setImporterName("sharad yadav");
	 ...
	 * (String)headerNames.nextElement(); System.out.println("Headername==" +
	 * paramName ); String paramValue = request.getHeader(paramName);
	 * System.out.println("Header Value==" +paramValue ); } String filePath="";
	 * ModelAndView mv = new ModelAndView();
	 * System.out.println("inside  update Consignment method"); try {
	...
	/*
	 * @RequestMapping(value=
	 * {"/openEditPage/{txnid}"},method={org.springframework.web.bind.annotation.
	 * RequestMethod.GET}) public ModelAndView
	 * openEditconsignmentPage(@PathVariable("txnid") String txnid) {
	 * System.out.println("inside edit page open method"+txnid); ModelAndView mv =
	 * new ModelAndView();
	 * 
	 * 
	 ...
	 * mv.addObject("consignmentdetails", consignmentdetails);
	 * System.out.println("consignemnt pojo details=**" +consignmentdetails);
	 * mv.setViewName("editConsignment"); return mv;
	 * 
	 * }
	 */
	...
	/*
	 * @RequestMapping(value=
	 * "/FillterConsignmentData",method={org.springframework.web.bind.annotation.
	 * RequestMethod.POST}) public ModelAndView
	 * FilterConsignmentData(@RequestParam(name = "startDate", required = false)
	 * String startDate ,@RequestParam(name="endDate",required =false) String
	 * endDate,
	 * 
	 * @RequestParam(name="fileStatus",required = false) String
	 * fileStatus,@RequestParam(name="taxStatus",required = false) String taxStatus,
	 * 
	 * @RequestParam("consignmentStatus") String consignmentStatus) {
	 * System.out.println("inside filter method");
	 * System.out.println("startDate= ("+startDate+") end date="
	 * +endDate+" file status="+fileStatus+" tax status="
	 * +taxStatus+"  consignmentStatus=="+consignmentStatus); ModelAndView mv = new
	 * ModelAndView(); mv.setViewName("demo");
	 * 
	 * 
	 * ConsignmentFilterPojo filterdata= new ConsignmentFilterPojo();
	 * filterdata.setEndDate(endDate); filterdata.setFileStatus(fileStatus);
	 * filterdata.setStartDate(startDate); filterdata.setTaxPaidStatus(taxStatus);
	 * filterdata.setConsignmentStatus(consignmentStatus);
	 * filterdata.setImporterId(1);
	 * 
	 * List<ConsignmentPojo>
	 * consignmentdetails=feignImpl.filterConsignmentdata(filterdata);
	 * mv.addObject("consignmentdetails", consignmentdetails);
	 * mv.addObject("startDate", startDate); mv.addObject("endDate", endDate);
	 * mv.addObject("fileStatus", fileStatus); mv.addObject("taxStatus", taxStatus);
	 ...
	 * ******************************************************************
	 * 
	 * @RequestMapping(value="/deleteConsignment",method={org.springframework.web.
	 * bind.annotation.RequestMethod.POST}) public @ResponseBody ModelAndView
	 * deleteconsignment(@RequestParam("txnId") String id) {
	 * System.out.println("inside delete method");
	 * System.out.println("transacation id="+id); ConsignmentPojo response=
	 * feignImpl.deleteConsignment(id);
	 * System.out.println("delete response="+response); ModelAndView mv = new
	 * ModelAndView();
	 * 
	 * mv.setViewName("redirect:/importerConsignment/1"); return mv;
	 * 
	 * }
	 */
	...
	/*
	 * @RequestMapping(value=
	 * "/dowloadFiles/{filetype}/{fileName}/{transactionNumber}",method={org.
	 * springframework.web.bind.annotation.RequestMethod.GET}) public String
	 * downloadFile(@PathVariable("transactionNumber") String
	 * txnid,@PathVariable("fileName") String fileName,@PathVariable("filetype")
	 * String filetype) throws IOException {
	 * System.out.println("inside file download method");
	 * System.out.println("transacation id="+txnid+" fileName="
	 * +fileName+" fileType="+filetype); String
	 * response=feignImpl.downloadFile(txnid,filetype,fileName);
	 ...
	 * 
	 * @RequestMapping(value={"/closeEditPage"},method={org.springframework.web.bind
	 * .annotation.RequestMethod.GET}) public ModelAndView closeEditPage() {
	 * 
	 * System.out.println("inside close edit page  method"); ModelAndView mv = new
	 * ModelAndView(); mv.setViewName("redirect:/importerConsignment/1"); return mv;
	 * 
	 * }
	 */
	...
	/*
	 * @RequestMapping(value={"/openReisterConsignmentPage"},method={org.
	 * springframework.web.bind.annotation.RequestMethod.GET}) public ModelAndView
	 * openRegisterConsignmentPage() {
	 * 
	 * System.out.println("inside open register consignment page  method");
	 * ModelAndView mv = new ModelAndView(); mv.setViewName("registerConsignment");
	 * return mv; }
	 * 
	 * 
	 * 
	 * 
	 * @RequestMapping(value={"/Home"},method={org.springframework.web.bind.
	 * annotation.RequestMethod.GET}) public ModelAndView Home() {
	 * System.out.println("inside home method"); ModelAndView mv = new
	 ...
	 * session.removeAttribute("username"); session.invalidate(); ModelAndView mv =
	 * new ModelAndView(); mv.setViewName("redirect:/register"); return mv; }
	 */
