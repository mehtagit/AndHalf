...
//	String filePath; 
	...
	public OtpResponse saveOtherRegistration(String data, MultipartFile photo, MultipartFile nationalIdImage,
			MultipartFile idCard, HttpSession session, HttpServletRequest request, MultipartFile vatFile)
			throws IOException {
		Gson gson = new Gson();
			...
			if (registration.getRePassword().equals(registration.getPassword())) {
				...
				if (nationalIdImage.isEmpty() == false) {
					...
					BufferedOutputStream bout = new BufferedOutputStream(
							new FileOutputStream(nationalIdPath + "/" + nationalIdImage.getOriginalFilename()));
					bout.write(barr);
					bout.flush();
					bout.close();
				...
				if (photo.isEmpty() == false) {
					...
					BufferedOutputStream bout = new BufferedOutputStream(
							new FileOutputStream(photoPath + "/" + photo.getOriginalFilename()));
					bout.write(barr);
					bout.flush();
					bout.close();
				...
				if (idCard.isEmpty() == false) {
					...
					BufferedOutputStream bout = new BufferedOutputStream(
							new FileOutputStream(idCardPath + "/" + idCard.getOriginalFilename()));
					bout.write(barr);
					bout.flush();
					bout.close();
				...
				if (registration.getUserTypeId() == 12) {
					...
					if (vatFile.isEmpty() == false) {
						...
						BufferedOutputStream bout = new BufferedOutputStream(
								new FileOutputStream(vatFilePath + "/" + vatFile.getOriginalFilename()));
						bout.write(barr);
						bout.flush();
						bout.close();
	...
	public OtpResponse saveRegistration(String data, MultipartFile file, MultipartFile vatFile, HttpSession session,
			...
			if (registration.getRePassword().equals(registration.getPassword())) {
				...
				if (registration.getType() == 0) {
					...
					if (file.isEmpty() == true) {
						...
						BufferedOutputStream bout = new BufferedOutputStream(
								new FileOutputStream(nationalIdPath + "/" + file.getOriginalFilename()));
						bout.write(barr);
						bout.flush();
						bout.close();
