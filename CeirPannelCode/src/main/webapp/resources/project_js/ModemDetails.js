			...
			$.ajax({
				...
				success: function(data){
					...
					for(i=0; i<date.length; i++){
						/*if(date[i].type === "date"){
						 ...
						 if(date[i].type === "text"){
							$("#PortTableDiv").append("<div class='input-field col s6 m2' ><input type="+date[i].type+" id="+date[i].id+" maxlength='20' /><label for="+date[i].id+" class='center-align'>"+date[i].title+"</label></div>");
						}
				...
				// dynamic dropdown portion
					...
					for(i=0; i<dropdown.length; i++){
						...
						$("#PortTableDiv").append("<div class='col s3 m2 l1'><button type='button'  class='btn primary botton' id='clearFilter'>Clear All Filter</button></div>");
