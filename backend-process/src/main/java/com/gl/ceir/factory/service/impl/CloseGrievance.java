...
public class CloseGrievance extends BaseService {
	...
	public void fetch() {
		User user = null;
		try {
			SystemConfigurationDb defaultPerioToCloseGrievance = this.systemConfigurationDbRepository
					.getByTag("default_period_to_close_grievance");
			...
			Optional<List<Grievance>> grievances = Optional.ofNullable(this.grievanceRepository.findAll(buildSpecification(toDate).build()));
			
			Map<String, String> placeholderMap = new HashMap<>();
			placeholderMap.put("<days>", defaultPerioToCloseGrievance.getValue());
			for (Grievance grievance : grievances.get()) {
				grievance.setGrievanceStatus(3);
				this.processedGrievances.add(grievance);
				this.grievanceHistories.add(new GrievanceHistory(grievance.getGrievanceId(),
						Long.valueOf(grievance.getUserId().intValue()), grievance.getUserType(),
						grievance.getGrievanceStatus(), grievance.getTxnId(), grievance.getCategoryId(),
						grievance.getFileName(), grievance.getRemarks(), Long.valueOf(-1L), "System"));
				...
				if (Objects.nonNull(user)) {
					...
					this.rawMails.add(new RawMail("Email", "MAIL_TO_USER_ON_GRIEVANCE_CLOSURE",
							grievance.getUserId().intValue(), 6L, "Process", "Closure", grievance.getTxnId(),
							"Closure Of Grievance With Transaction Number " + grievance.getTxnId(), placeholderMap,
							"USERS", grievance.getUserType(), grievance.getUserType()));
					continue;
				}
