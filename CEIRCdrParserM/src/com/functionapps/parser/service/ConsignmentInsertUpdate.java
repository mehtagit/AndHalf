...
//            if ((sub_feature.equalsIgnoreCase("register") || sub_feature.equalsIgnoreCase("update") || sub_feature.equalsIgnoreCase("upload"))) {
            ...
            fileErrorLines.add(" DEVICETYPE,DeviceIdType,MultipleSIMStatus,S/NofDevice,IMEI/ESN/MEID,Devicelaunchdate,DeviceStatus, Error Code ,Error Message ");
//                errFile.gotoErrorFile(txn_id, "DEVICETYPE,DeviceIdType,MultipleSIMStatus,S/NofDevice,IMEI/ESN/MEID,Devicelaunchdate,DeviceStatus, Error Code ,Error Message ");
            while (rs.next()) {
                ...
                my_rule_detail = rule_filter.getMyFeatureRule(conn, device_info, rulelist, myWriter, fileErrorLines);
    ...
    private void insertFromImporterManufactor(Connection conn, ResultSet rs1, HashMap<String, String> stolnRcvryDetails, HashMap<String, String> feature_file_management, HashMap<String, String> feature_file_mapping, String dateNow, String period, String txn_id) {
        ...
        try {
            String qry = " select a.imei_esn_meid from device_importer_db as a , device_importer_db as b  where a.sn_of_device = b.sn_of_device "
                    ...
                    + "select a.imei_esn_meid from device_manufacturer_db as a , device_manufacturer_db as b  where a.sn_of_device = b.sn_of_device and"
            ...
            while (rs.next()) {
                ...
                if (stolnRcvryDetails.get("operation").equals("0")) {
                    device_greylist_db_qry = "insert into   greylist_db (created_on , imei, user_id , txn_id , mode_type  , request_type, user_type  , complain_type)   "
                            + "values(           " + "'" + dateNow + "'," + "'" + rs.getString("imei_esn_meid")
                            + "'," + " ( select username from users where users.id=  "
                            + feature_file_management.get("user_id") + "  )  ,  " + " '" + txn_id + "', " + "'"
                            + stolnRcvryDetails.get("source") + "'," + "'" + stolnRcvryDetails.get("reason")
                            + "'," + "'" + stolnRcvryDetails.get("usertype") + "'," + "'"
