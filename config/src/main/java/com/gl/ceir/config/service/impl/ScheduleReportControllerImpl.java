    ...
    GenericSpecificationBuilder<ScheduleReportDb> buildSpecification(FilterRequest filterRequest) {
        ...
        if (Objects.nonNull(filterRequest.getFlag())) {
          ...
          if (Objects.nonNull(filterRequest.getCreatedOn())) {
            cmsb.with(new SearchCriteria("createdOn", filterRequest.getCreatedOn(), SearchOperation.EQUALITY, Datatype.STRING));
        }
            if (Objects.nonNull(filterRequest.getModifiedOn())) {
            cmsb.with(new SearchCriteria("modifiedOn", filterRequest.getModifiedOn(), SearchOperation.EQUALITY, Datatype.STRING));
        }
              if (Objects.nonNull(filterRequest.getReportName())) {
            cmsb.with(new SearchCriteria("reportName", filterRequest.getReportName(), SearchOperation.EQUALITY, Datatype.STRING));
        }
        ...
        if (Objects.nonNull(filterRequest.getSearchString()) && !filterRequest.getSearchString().isEmpty()) {
            ...
            cmsb.orSearch(new SearchCriteria("createdOn", filterRequest.getSearchString(), SearchOperation.LIKE, Datatype.STRING));
            cmsb.orSearch(new SearchCriteria("modifiedOn", filterRequest.getSearchString(), SearchOperation.LIKE, Datatype.STRING));
            cmsb.orSearch(new SearchCriteria("reportName", filterRequest.getSearchString(), SearchOperation.LIKE, Datatype.STRING));
        }
        return cmsb;
    }
